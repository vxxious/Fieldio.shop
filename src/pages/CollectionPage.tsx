...
export function CollectionPage() {
  ...
  return <div className="collection-page">
    ...
    <div className="collection-filters"><label>Brand<select value={brand} onChange={(e) => update("brand", e.target.value)}><option value="">All brands</option>{brands.map((value) => <option key={value}>{value}</option>)}</select></label><label>Size<select value={size} onChange={(e) => update("size", e.target.value)}><option value="">All sizes</option>{sizes.map((value) => <option key={value}>{value}</option>)}</select></label><label>Sort<select value={sort} onChange={(e) => update("sort", e.target.value)}><option value="featured">Featured</option><option value="name">Name</option><option value="price-low">Price: low to high</option><option value="price-high">Price: high to low</option></select></label>{(brand || size) && <button className="text-link" onClick={() => setParams({ sort })}>Clear filters</button>}<p aria-live="polite">{visibleProducts.length} {visibleProducts.length === 1 ? "piece" : "pieces"}</p></div>
    {error ? <div className="empty-state" role="alert"><h2>The edit could not load.</h2><button className="primary-button" onClick={() => void refetch()}>Try again</button></div> : isLoading ? <div className="product-grid">{Array.from({ length: 4 }).map((_, index) => <div className="product-skeleton" key={index} />)}</div> : visibleProducts.length ? <section className="product-grid"><h2 className="sr-only">Products</h2>{visibleProducts.map((product) => <ProductCard key={product.id} product={product} />)}</section> : <div className="empty-state"><h2>No pieces in this edit yet.</h2><p>Adjust your filters or ask a Fieldio personal shopper to source something specific.</p></div>}
  </div>;
}
