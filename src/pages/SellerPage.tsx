...
interface SellerApplication { id: string; kind: "seller" | "vendor"; legal_name: string; business_name: string | null; country_code: string; phone: string; contact_email: string; website: string | null; identity_document_path: string; address_document_path: string; business_document_path: string | null; status: ReviewStatus; review_reason: string | null; }
interface Store { id: string; name: string; slug: string; description: string; contact_email: string; contact_phone: string; status: "active" | "suspended"; }
...
const applicationSchema = z.object({
  ...
  phone: z.string().trim().min(7, "Enter a valid phone number.").max(30), contact_email: z.string().email("Enter a valid email."),
...
const storeSchema = z.object({ name: z.string().trim().min(2).max(120), description: z.string().trim().min(20, "Add at least 20 characters.").max(2000), contact_email: z.string().email(), contact_phone: z.string().trim().min(7).max(30) });
...
const listingSchema = z.object({
  ...
  category_id: z.string().uuid("Choose a category."), subcategory_id: z.string(), condition: z.enum(["new_with_tags", "new_without_tags", "excellent", "good", "fair"]),
  price: z.number().positive("Enter a price."), compare_at_price: z.union([z.literal(""), z.number().positive()]), currency: z.string().length(3),
  colors: z.string().max(300), sizes: z.string().max(300), quantity: z.number().int().min(1).max(10000), weight_kg: z.number().positive().max(1000)
...
function VerificationForm({ userId, email, application, onDone }: { userId: string; email: string; application: SellerApplication | null; onDone: () => Promise<void> }) {
  ...
  const { control, register, handleSubmit, setFocus, formState: { errors, isSubmitting } } = useForm<ApplicationValues>({ resolver: zodResolver(applicationSchema), defaultValues: {
    kind: application?.kind ?? "seller", legal_name: application?.legal_name ?? "", business_name: application?.business_name ?? "", country_code: application?.country_code ?? "", phone: application?.phone ?? "", contact_email: application?.contact_email ?? email, website: application?.website ?? "", declaration: false
  } });
  ...
  return <section className="seller-panel seller-verification"><header><h1>Seller verification</h1><p>Fieldio reviews every seller before a store can publish. Your documents stay private and are used only for verification.</p></header>
    ...
    <form className="seller-form" onSubmit={handleSubmit(submit, (formErrors) => setFocus(Object.keys(formErrors)[0] as keyof ApplicationValues))} noValidate>
      ...
      <div className="seller-form-row"><label><span>Country code</span><input placeholder="GB" maxLength={2} {...register("country_code")} aria-invalid={!!errors.country_code} aria-describedby={errors.country_code ? "seller-country-error" : undefined} /><FieldError id="seller-country-error" message={errors.country_code?.message} /></label><label><span>Phone</span><input type="tel" autoComplete="tel" {...register("phone")} aria-invalid={!!errors.phone} aria-describedby={errors.phone ? "seller-phone-error" : undefined} /><FieldError id="seller-phone-error" message={errors.phone?.message} /></label></div>
...
function StoreForm({ userId, email, onDone }: { userId: string; email: string; onDone: () => Promise<void> }) {
  const [status, setStatus] = useState("");
  const { register, handleSubmit, setFocus, formState: { errors, isSubmitting } } = useForm<StoreValues>({ resolver: zodResolver(storeSchema), defaultValues: { contact_email: email } });
  async function submit(values: StoreValues) {
    setStatus("");
    const slug = `${slugify(values.name) || "store"}-${userId.slice(0, 6)}`;
    const { error } = await supabase!.from("seller_stores").insert({ owner_id: userId, ...values, slug });
  ...
  return <section className="seller-panel"><header><StoreIcon /><h1>Create your store</h1><p>Your approved store name becomes the brand shown on your products.</p></header><form className="seller-form" onSubmit={handleSubmit(submit, (formErrors) => setFocus(Object.keys(formErrors)[0] as keyof StoreValues))} noValidate>
    ...
    <div className="seller-form-row"><label><span>Contact email</span><input type="email" {...register("contact_email")} aria-invalid={!!errors.contact_email} /><FieldError id="store-email-error" message={errors.contact_email?.message} /></label><label><span>Contact phone</span><input type="tel" {...register("contact_phone")} aria-invalid={!!errors.contact_phone} /><FieldError id="store-phone-error" message={errors.contact_phone?.message} /></label></div>
...
function ListingForm({ userId, store, categories, currency, onCancel, onSubmitted }: { userId: string; store: Store; categories: Category[]; currency: string; onCancel: () => void; onSubmitted: (title: string) => Promise<void> }) {
  ...
  const { control, register, handleSubmit, getValues, setFocus, formState: { errors, isSubmitting } } = useForm<ListingValues>({ resolver: zodResolver(listingSchema), defaultValues: { condition: "excellent", currency, quantity: 1, colors: "", sizes: "", compare_at_price: "" } });
  const selectedCategory = useWatch({ control, name: "category_id" });
  const subcategories = categories.filter((item) => item.parent_id === selectedCategory);
  ...
  return <section className="seller-panel"><header><button type="button" className="text-link" onClick={onCancel}>← Seller dashboard</button><h1>Add a product</h1><p>Listings are checked for authenticity, ownership, condition, and image quality before they appear in the shop.</p></header><form className="seller-form" onSubmit={handleSubmit(() => { if (!images.length) { setStatus("Add at least one clear product image."); return; } setReviewing(true); }, (formErrors) => setFocus(Object.keys(formErrors)[0] as keyof ListingValues))} noValidate>
    ...
    <div className="seller-form-row"><label><span>Condition</span><select {...register("condition")}><option value="new_with_tags">New with tags</option><option value="new_without_tags">New without tags</option><option value="excellent">Excellent</option><option value="good">Good</option><option value="fair">Fair</option></select></label><label><span>Currency</span><input maxLength={3} {...register("currency")} /></label></div>
    <div className="seller-form-row seller-price-row"><label><span>Selling price</span><input type="number" min="0.01" step="0.01" {...register("price", { valueAsNumber: true })} aria-invalid={!!errors.price} /><FieldError id="listing-price-error" message={errors.price?.message} /></label><label><span>Original price <small>Optional slash price</small></span><input type="number" min="0.01" step="0.01" {...register("compare_at_price", { setValueAs: (value) => value === "" ? "" : Number(value) })} aria-invalid={!!errors.compare_at_price} /><FieldError id="listing-compare-error" message={errors.compare_at_price?.message} /></label></div>
    <div className="seller-form-row"><label><span>Colours <small>Comma separated</small></span><input placeholder="Black, Ivory" {...register("colors")} /></label><label><span>Sizes <small>Comma separated</small></span><input placeholder="S, M, L" {...register("sizes")} /></label></div>
    <div className="seller-form-row"><label><span>Quantity</span><input type="number" min="1" {...register("quantity", { valueAsNumber: true })} aria-invalid={!!errors.quantity} /><FieldError id="listing-quantity-error" message={errors.quantity?.message} /></label><label><span>Weight (kg)</span><input type="number" min="0.001" step="0.001" {...register("weight_kg", { valueAsNumber: true })} aria-invalid={!!errors.weight_kg} /><FieldError id="listing-weight-error" message={errors.weight_kg?.message} /></label></div>
    {status && <p className="form-message" role="alert">{status}</p>}<button className="primary-button">Review listing</button>
  </form></section>;
}

function SellerDashboard({ store, listings, onAdd }: { store: Store; listings: Listing[]; onAdd: () => void }) {
  return <section className="seller-dashboard"><header><div><p>{store.name}</p><h1>Seller dashboard</h1><span>{store.description}</span></div><button className="primary-button" onClick={onAdd}><PlusIcon /> Add product</button></header><div className="seller-summary"><div><strong>{listings.length}</strong><span>Total listings</span></div><div><strong>{listings.filter((item) => item.status === "pending").length}</strong><span>In review</span></div><div><strong>{listings.filter((item) => item.status === "approved").length}</strong><span>Live</span></div></div><section className="seller-listings"><h2>Your products</h2>{listings.length ? listings.map((listing) => <article key={listing.id}><div><h3>{listing.title}</h3><p>{listing.currency} {(listing.price / 100).toLocaleString(undefined, { minimumFractionDigits: 2 })} · <span className={`seller-status seller-status--${listing.status}`}>{listing.status}</span></p>{listing.review_reason && <small>{listing.review_reason}</small>}</div>{listing.published_product_id && <Link className="text-link" to="/collections">View in shop <ArrowIcon /></Link>}</article>) : <div className="seller-empty"><StoreIcon /><h3>Your store is ready.</h3><p>Add your first product. Fieldio will review it before publication.</p><button className="text-link" onClick={onAdd}>Add a product</button></div>}</section></section>;
}
...
export function SellerPage() {
  ...
  const [mode, setMode] = useState<"dashboard" | "listing">("dashboard");
  ...
  if (!application || application.status === "draft" || application.status === "rejected") return <VerificationForm userId={session.user.id} email={session.user.email ?? ""} application={application} onDone={refresh} />;
  ...
  if (!store) return <StoreForm userId={session.user.id} email={application.contact_email} onDone={refresh} />;
  ...
  return <SellerDashboard store={store} listings={listings} onAdd={() => setMode("listing")} />;
}
