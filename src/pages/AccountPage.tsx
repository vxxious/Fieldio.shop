...
export function AccountPage() {
  ...
  return <div className="account-page"><section><h1>{titles[mode]}</h1><p>Manage saved details, wishlist, and order requests.</p><form onSubmit={handleSubmit(onSubmit)} noValidate>
    {mode !== "update" && <label><span>Email</span><input type="email" autoComplete="email" {...register("email")} aria-invalid={!!errors.email} />{errors.email && <small role="alert">{errors.email.message}</small>}</label>}
  ...
  </form>{mode === "signin" && <div className="oauth-buttons"><button type="button" onClick={() => void oauth()}>Continue with Google</button></div>}
