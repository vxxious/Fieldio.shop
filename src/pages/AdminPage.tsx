...
export function AdminPage() {
  ...
  const selectSection = (nextSection: string) => {
    if (nextSection === resource.table || !hasUnsavedChanges || window.confirm("Discard unsaved changes?")) setSection(nextSection);
  };
  return <div className="admin-page"><aside><h1>Fieldio Admin</h1><nav aria-label="Administration">{resources.map((item) => <button key={item.table} aria-current={resource.table === item.table ? "page" : undefined} onClick={() => selectSection(item.table)}>{item.title}</button>)}</nav></aside><AdminWorkspace key={resource.table} resource={resource} onDirtyChange={handleDirtyChange} /></div>;
}
