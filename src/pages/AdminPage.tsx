...
export function AdminPage() {
  ...
  return <div className="admin-page"><aside><h1>Fieldio Admin</h1><nav aria-label="Administration">{resources.map((item) => <button key={item.table} aria-current={resource.table === item.table ? "page" : undefined} onClick={() => setSection(item.table)}>{item.title}</button>)}</nav></aside><AdminWorkspace key={resource.table} resource={resource} /></div>;
}
