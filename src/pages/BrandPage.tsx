...
import { brands as sourcingBrands } from "../data/catalog";
...
export function BrandPage() {
  ...
  const exactBrand = slug === "louis-vuitton" ? "Louis Vuitton" : brand;
  ...
  if (!slug) return <section className="brand-page"><header><h1>Brands</h1><p>Explore the labels in our edit, or ask Fieldio to source a specific piece.</p></header><nav className="brand-list" aria-label="Brands">{[...new Set([...sourcingBrands.filter((name) => name !== "Fieldio Edit"), ...products.map((product) => product.brand)])].map((name) => <Link key={name} to={`/brands/${name.toLowerCase().replaceAll(" ", "-")}`}>{name}</Link>)}</nav></section>;
