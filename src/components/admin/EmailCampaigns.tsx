...
export function EmailCampaigns() {
  ...
  const ctaIssue = actionLabel && !actionUrl
    ? (/^https?:\/\//i.test(actionLabel) ? "Move the web address to Button URL, then enter a short label such as Shop now." : "Add the Button URL, or clear the Button label.")
    : actionUrl && !actionLabel ? "Add a Button label such as Shop now, or clear the Button URL." : "";
  const validDraft = () => {
    const valid = formRef.current?.reportValidity() ?? false;
    if (!valid) setNotice({ message: ctaIssue || "Complete the highlighted campaign fields and try again.", error: true });
    return valid;
  };
  async function run<T>(action: () => Promise<T>, success: string) {
    setWorking(true); setNotice(undefined);
    try { const result = await action(); setNotice({ message: success, error: false }); return result; }
    catch (error) { setNotice({ message: error instanceof Error ? error.message : "The campaign action failed.", error: true }); }
    finally { setWorking(false); }
  }
  const persistDraft = () => authenticatedPost<{ campaign: CampaignRow }>("/api/admin/campaigns", { action: "save", campaignId, campaign: draft });
  async function save(event: FormEvent) {
    event.preventDefault();
    const result = await run(persistDraft, "Draft saved.");
  ...
  async function sendTest() {
    if (!validDraft()) return;
    if (!testEmailRef.current?.reportValidity()) { setNotice({ message: "Enter a valid test recipient email.", error: true }); return; }
  ...
  async function send() {
    ...
    const result = await run(async () => {
      ...
      setCampaignId(saved.campaign.id);
      return authenticatedPost<{ sent: number; failed: number }>("/api/admin/campaigns", { action: "send", campaignId: saved.campaign.id });
    }, "Campaign sent.");
    if (result) { setNotice({ message: `Resend accepted ${result.sent} messages${result.failed ? `; ${result.failed} failed and can be retried` : ""}.`, error: Boolean(result.failed) }); await cache.invalidateQueries({ queryKey: ["admin-campaigns"] }); }
  }
  return <section className="admin-campaigns">
    <header><div><h2>Email campaigns</h2><p>Create a restrained Fieldio campaign and send only to subscribed audiences.</p></div><button className="secondary-button" type="button" onClick={() => { setCampaignId(undefined); setDraft(emptyCampaign); setPreview(""); setRecipientCount(undefined); setNotice(undefined); }}>New campaign</button></header>
    <div className="campaign-layout"><div className="campaign-editor"><form ref={formRef} className="admin-form campaign-form" onSubmit={(event) => void save(event)}>
      ...
      <label><span>Button label (optional)</span><input minLength={2} maxLength={80} required={Boolean(actionUrl)} aria-invalid={Boolean(ctaIssue)} aria-describedby={ctaIssue ? "campaign-cta-error" : undefined} placeholder="Shop now" value={draft.actionLabel} onChange={(event) => set("actionLabel", event.target.value)} /></label>
      <label><span>Button URL (optional)</span><input type="url" required={Boolean(actionLabel)} aria-invalid={Boolean(ctaIssue)} aria-describedby={ctaIssue ? "campaign-cta-error" : undefined} placeholder="https://fieldio.shop/…" value={draft.actionUrl} onChange={(event) => set("actionUrl", event.target.value)} /></label>
      {ctaIssue && <small id="campaign-cta-error" className="field-error campaign-cta-error">{ctaIssue}</small>}
      <div className="campaign-actions wide"><button className="secondary-button" disabled={working || locked}>{working ? "Working…" : locked ? "Campaign sent" : "Save draft"}</button><button className="secondary-button" type="button" disabled={working || locked} onClick={() => void showPreview()}>Preview audience</button><button className="primary-button" type="button" disabled={working || locked} onClick={() => void send()}>Save &amp; send campaign</button></div>
    </form><div className="campaign-test"><label><span>Test recipient email</span><input ref={testEmailRef} type="email" required value={testEmail} onChange={(event) => setTestEmail(event.target.value)} placeholder="Enter an email address" /></label><button className="secondary-button" type="button" disabled={working || locked} onClick={() => void sendTest()}>Send test email</button></div>{notice && <p className={notice.error ? "field-error campaign-notice" : "form-message campaign-notice"} role={notice.error ? "alert" : "status"}>{notice.message}</p>}</div>
