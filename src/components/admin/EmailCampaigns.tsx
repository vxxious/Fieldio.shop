...
export function EmailCampaigns() {
  ...
  return <section className="admin-campaigns">
    ...
    <div className="campaign-layout"><form className="admin-form campaign-form" onSubmit={(event) => void save(event)} noValidate>
      ...
      <label><span>Button label (optional)</span><input maxLength={80} value={draft.actionLabel} onChange={(event) => set("actionLabel", event.target.value)} /></label>
      <label><span>Button URL (optional)</span><input type="url" placeholder="https://fieldio.shop/…" value={draft.actionUrl} onChange={(event) => set("actionUrl", event.target.value)} /></label>
      ...
      <div className="campaign-test wide"><label><span>Test recipient</span><input type="email" value={testEmail} onChange={(event) => setTestEmail(event.target.value)} placeholder="you@fieldio.shop" /></label><button className="text-link" type="button" disabled={working || !testEmail} onClick={() => void run(() => authenticatedPost("/api/admin/campaigns", { action: "test", email: testEmail, campaign: draft }), "Test email sent.")}>Send test</button></div>
      {status && <p className="form-message wide" role="status">{status}</p>}
    </form>
