...
const detailsSchema = z.object({ full_name: z.string().trim().min(2).max(120), phone: z.string().trim().min(7).max(30), line1: z.string().trim().min(3).max(150), city: z.string().trim().min(2).max(100), postal_code: z.string().trim().max(30), country_code: z.string().trim().length(2).transform((v) => v.toUpperCase()) });
...
export function AccountDetails({ userId, email }: { userId: string; email: string }) {
  ...
  const details = useQuery({ queryKey: ["account-details", userId], queryFn: async () => {
    const [profile, address] = await Promise.all([supabase!.from("profiles").select("full_name,phone").eq("id", userId).single(), supabase!.from("addresses").select("*").eq("user_id", userId).eq("is_default", true).maybeSingle()]);
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<Details>({ resolver: zodResolver(detailsSchema), values: {
    full_name: details.data?.profile.full_name || "", phone: details.data?.profile.phone || "", line1: details.data?.address?.line1 || "", city: details.data?.address?.city || "", postal_code: details.data?.address?.postal_code || "", country_code: details.data?.address?.country_code || ""
  } });
  ...
  return <div className="customer-account"><header><h1>Your account</h1><p>{email}</p><Link to="/wishlist" className="text-link">Your wishlist</Link><button className="text-link" onClick={async () => { const { error } = await supabase!.auth.signOut(); if (error) setStatus("Sign out failed. Please try again."); else cache.clear(); }}>Sign out</button></header>
    <section id="orders"><h2>Order requests</h2>{orders.isPending ? <p role="status">Loading requests…</p> : orders.error ? <p role="alert">Requests could not load. <button onClick={() => void orders.refetch()}>Try again</button></p> : orders.data.length ? orders.data.map((order) => <article className="account-order" key={order.id}><h3>{order.public_reference}</h3><p>{order.status.replaceAll("_", " ")} · {new Date(order.created_at).toLocaleDateString("en-GB")}</p><ul>{order.order_items.map((item) => <li key={item.id}>{item.product_name} · {item.size} · Quantity {item.quantity}</li>)}</ul></article>) : <p>No order requests yet. Requests made while signed in will appear here.</p>}</section>
    <section id="details"><h2>Saved information</h2>{details.error ? <p role="alert">Details could not load. <button onClick={() => void details.refetch()}>Try again</button></p> : <form className="admin-form" onSubmit={handleSubmit(save)} noValidate>{([['full_name','Full name'],['phone','Phone number'],['line1','Street address'],['city','City'],['postal_code','Postal code'],['country_code','Country code (GB, US, etc.)']] as const).map(([key,label]) => <label key={key}><span>{label}</span><input {...register(key)} aria-invalid={!!errors[key]} />{errors[key] && <small role="alert">{errors[key]?.message}</small>}</label>)}<button className="primary-button" disabled={isSubmitting || details.isPending}>Save details</button></form>}{status && <p role="status">{status}</p>}</section>
  </div>;
}
