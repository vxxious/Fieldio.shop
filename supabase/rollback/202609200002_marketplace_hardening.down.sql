...
update public.products set inquiry_only = true where seller_listing_id is not null;
update public.inventory inventory
set allow_backorder = true
from public.product_variants variant
join public.products product on product.id = variant.product_id
where inventory.variant_id = variant.id and product.seller_listing_id is not null;

comment on function public.has_admin_role(text[]) is null;
comment on function public.is_admin() is null;
comment on function public.is_owner() is null;
