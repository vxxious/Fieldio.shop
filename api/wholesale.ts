...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    await Promise.all([
      sendTransactionalEmail({ to: input.email, subject: "Fieldio wholesale enquiry received", heading: "We received your wholesale enquiry", message: "Thank you. Fieldio will review the details and contact you as soon as possible.", details: [{ label: "Subject", value: input.subject }] }),
      sendTransactionalEmail({ to: notificationEmail(), replyTo: input.email, subject: "New Fieldio wholesale enquiry", heading: "New wholesale enquiry", message: input.message, details: [{ label: "Subject", value: input.subject }, { label: "Name", value: input.name }, { label: "Company", value: input.company || "Not supplied" }, { label: "Email", value: input.email }, { label: "Phone", value: input.phone }] })
    ]);
