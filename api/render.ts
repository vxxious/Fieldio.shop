...
export async function GET(request: Request) {
  ...
  try {
    ...
    const html = template.replace(/<title>[\s\S]*?<\/title>/i, "").replace(/<meta\s+(?:name|property)=["'](?:description|og:[^"']+|twitter:[^"']+)["'][^>]*>/gi, "").replace(/<link\s+rel=["']canonical["'][^>]*>/gi, "").replace(/<script id=["']fieldio-identity["'][\s\S]*?<\/script>/i, "").replace("</head>", `${head}</head>`).replace(/<div id=["']root["']>[\s\S]*?<\/div>(?=\s*<\/body>)/i, privateRoute ? loader : crawlableShell(heading, description));
