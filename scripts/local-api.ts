  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use(async (request, response, next) => {
        ...
        if (!/^\/api\/(order-requests|seller-listing-review|newsletter|newsletter-preferences|contact|wholesale|sitemap|render|locale|rates)$/.test(url.pathname)) { next(); return; }
        try {
